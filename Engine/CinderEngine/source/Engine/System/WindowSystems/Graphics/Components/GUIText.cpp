...
namespace Framework
{
	...
	void GUIText::Draw()
	{
		...
		if (visible)
		{
			...
			shader->uniMat4("mvp", glm::value_ptr(Camera::main->GetViewToProjectionMatrix () * Camera::main->GetWorldToViewMatrix () * tform->GetModelMatrix()));
  ...
  void GUIText::Draw()
  {
    ...
    if (visible)
    {
      Transform* tform = static_cast<Transform*>(gameObject->GetComponent("Transform"));
		...
		// DAMIEN NOTE: Manas told me to change this line to make it work
